...
import GLBAnimationActioner, {
    GLBAnimationAttributes,
} from './GLBAnimationActioner';
...
export default class AnimationContextWrapper {
    ...
    constructor(frame: number, script: Script, language: string) {
        this.frame = frame;
        this.script = script;
        const { subtitle, endTime } =
            this.script.getSubtitleAndItsEndTimeAt(frame);
    ...
    getGLBAnimationAttributes(actor: string): GLBAnimationAttributes {
        const result = this.getActionOfActor(actor)
            ...
            .reduce((prev, curr) => curr.combine(prev), {
                ...GLBAnimationActioner.defaultValue,
            });
    ...
    get3DObjectStateOf(actor: string): ThreeDObjectState {
        return this.getActionOfActor(actor)
            ...
            .reduce(
                (prev, curr) => curr.combine(prev),
                ObjectActioner.defaultValue,
            )
    ...
    getConnectors(): ConnectorStates {
        return this.getActionByType('connect to')
            ...
            .reduce(
                (prev, curr) => curr.combine(prev),
                ConnectorsActioner.defaultValue,
            )
            .getConnectors(this.adjustedFrame, this.script.fps);
    }

    private isSubtitleWithFlashBack(
        subtitle: Subtitle,
    ): subtitle is SubtitleWithFlashBack {
        return (
            subtitle &&
            (subtitle as SubtitleWithFlashBack).flashBack !== undefined
        );
    }
    ...
    private get adjustedFrame(): number {
        if (this.isSubtitleWithFlashBack(this.currentSubtitle)) {
            ...
            const startTime =
                this.currentSubtitleEndTime - this.currentSubtitle.duration;
            const actualTimeWithInSubtitle =
                this.frame / this.script.fps - startTime;
            if (actualTimeWithInSubtitle < flashBack.duration) {
                const resulttime =
                    actualTimeWithInSubtitle * flashBack.speed + flashBack.from;
    ...
    getCurrentSubtitleText() {
        ...
        const translation =
            subtitle.translations && this.language
                ? subtitle.translations[this.language]
                : undefined;
        ...
        return {
            ...
            text:
                !subtitle ||
                this.frame > endTime * this.script.fps ||
                this.frame < (endTime - subtitle.duration) * this.script.fps
                    ? ''
                    : text,
        };
    }
